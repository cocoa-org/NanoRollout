"""Terminus-2 (Terminal Bench) runner entry point."""
...
def run_tb_terminus2(
    ...
    parser_name = extra_args.get("parser_name", "json")
    ...
    try:
        ...
        # Parse sampling params for Terminus-2
        sp = sampling_params
        ...
        with env_logging(trial_dir):
            with trial_logging(trial_dir):
                ...
                logger.info(
                    "[%s] Running Terminus-2 agent (parser=%s)",
                    instance_id,
                    parser_name,
                )
                agent_start = time.time()
                from brew.harness.agents.terminal.terminus2 import Terminus2Agent
                ...
                agent = Terminus2Agent(
                    ...
                    extra_body=sp.get("extra_body", {}),
                    enable_asciinema=extra_args.get("enable_asciinema", False),
                    output_dir=str(trial_dir),
                    enable_summarize=extra_args.get("enable_summarize", True),
                    proactive_summarization_threshold=extra_args.get(
                        "proactive_summarization_threshold", 8000
                    ),
                    ubuntu_mirror=extra_args.get("ubuntu_mirror", "us.archive.ubuntu.com"),
                    ubuntu_mirror_apt_update=extra_args.get("ubuntu_mirror_apt_update", True),
                )
                agent_result = agent.run(instruction, total_timeout=agent_timeout)
        ...
        error_msg = str(exc)
        logger.exception("Terminus-2 run failed for %s", instance_id)
    finally:
        if env_obj:
            env_obj.stop()
