"""Docker-backed sandbox runtime for UDA tasks.

Builds and starts a single task's container via ``docker compose``
against the per-task ``docker-compose.yaml`` shipped in the migrated
task corpus (``adapter/<benchmark>/<task_id>/``). The task's
``Dockerfile`` is FROM the unified ``uda-desktop`` base image, so the
runtime layer here is identical to cocoa_env.docker.
"""
...
class DockerComposeSandboxRuntime(BaseSandboxRuntime):
    ...
    def start(self, task: Dict[str, Any], wait_time: int = 60) -> bool:
        try:
            ...
            env = {
                "TASK_DOCKER_IMAGE_NAME": f"uda-{bench}-{task_name}:latest",
                "TASK_DOCKER_CONTAINER_NAME": f"uda-{bench}-{task_name}-container",
    ...
    def cleanup(self) -> bool:
        try:
            if self.client.task_dir and self.client.task_name:
                ...
                bench = (self.client.sandbox_config.get("bench") or "uda").strip() or "uda"
                env = {
                    "TASK_DOCKER_IMAGE_NAME": f"uda-{bench}-{self.client.task_name}:latest",
                    "TASK_DOCKER_CONTAINER_NAME": f"uda-{bench}-{self.client.task_name}-container",
                    "HOST_PORT": str(self.client.port),
                }
                runtime_logger.info(
