"""Cocoa sandbox clients and shared runtime primitives."""
...
class SandboxClient:
    ...
    def __init__(self, sandbox_config: Dict[str, Any] | None = None, **kwargs):
        ...
        base_url = self.sandbox_config.get("base_url", kwargs.get("base_url", f"http://localhost:{self.port}"))
...
class BrowserSandboxClient(SandboxClient):
    ...
    def _reset_sdk_client_state(self) -> None:
        ...
        self._cached_browser_viewport = None
    ...
    def _normalize_cdp_url(self, cdp_url: str) -> str:
        """Adjust CDP URL scheme for proxied HTTPS runtimes like Modal tunnels."""
        if not cdp_url:
            return cdp_url

        if self.base_url.startswith("https://") and cdp_url.startswith("ws://"):
            normalized = "wss://" + cdp_url[len("ws://"):]
            logger.info("Normalized CDP URL for HTTPS tunnel: %s -> %s", cdp_url, normalized)
            return normalized
    ...
    def _with_page(self, func, wait_timeout: int = 5000):
        ...
        async def runner():
            ...
            async with async_playwright() as p:
                browser = await p.chromium.connect_over_cdp(self._normalize_cdp_url(browser_info.cdp_url))
                try:
