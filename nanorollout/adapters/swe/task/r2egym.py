...
def setup_r2egym_env(
    env: Any, workspace_dir: str = "/testbed", alt_path: str = "/root"
) -> None:
    ...
    env.execute(
        f"ln -sf {workspace_dir}/.venv/bin/python {alt_path}/.local/bin/python3"
    )
    env.execute(
    ...
    for skip_file in _R2E_SKIP_FILES:
        env.execute(
            f"mv {workspace_dir}/{skip_file} {alt_path}/{skip_file} 2>/dev/null"
        )
