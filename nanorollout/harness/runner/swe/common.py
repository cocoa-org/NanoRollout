...
from nanorollout.harness.agents.swe.base import AgentConfig
...
def build_eval_payload(instance: Dict[str, Any], output: str) -> Dict[str, Any]:
    ...
    from nanorollout.eval.swebench.grading import get_eval_report
...
def _run_eval(
    ...
    if _resolve_naming_strategy(dataset) == NamingStrategy.R2E_GYM:
        from nanorollout.eval.r2egym.grading import run_r2egym_eval

        return run_r2egym_eval(env_obj, instance, eval_timeout)

    try:
        logger.info("[%s] Generating test spec for eval", instance_id)
        from nanorollout.eval.swebench.test_spec import make_test_spec
