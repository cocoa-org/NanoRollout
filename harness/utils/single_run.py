...
from harness.utils.artifacts import DATASET_MAPPING
...
def create_environment(
    ...
    if env_type == "docker":
        from harness.envs.docker import DockerEnvironment
    ...
    if env_type == "enroot":
        from harness.envs.enroot import EnrootEnvironment
    ...
    if env_type == "modal":
        from harness.envs.modal import ModalEnvironment
...
def build_eval_payload(instance: Dict[str, Any], output: str) -> Dict[str, Any]:
    from harness.eval.swebench.constants import (
        END_TEST_OUTPUT,
        START_TEST_OUTPUT,
        ResolvedStatus,
    )
    from harness.eval.swebench.grading import get_eval_report
